...
const HistoryPopover: React.FC<HistoryPopoverProps> = ({ isOpen, onClose }) => {
  ...
  const [activeItem, setActiveItem] = useState<EnrichedHistoryItem | null>(null);
  const [copiedTimestamp, setCopiedTimestamp] = useState<number | null>(null);
  const [copiedSelectorTimestamp, setCopiedSelectorTimestamp] = useState<number | null>(null);
  ...
  const handleCopySelector = async (e: React.MouseEvent, item: EnrichedHistoryItem) => {
    e.stopPropagation();
    try {
  ...
  const handleCopyText = async (e: React.MouseEvent, item: EnrichedHistoryItem) => {
    e.stopPropagation();
    try {
      if (item.innerText) {
  ...
  const handleRemove = async (e: React.MouseEvent, item: EnrichedHistoryItem) => {
    e.stopPropagation();
  ...
  return (
    <div className='fixed bottom-[6.5em] left-6 z-[2147483647] rounded-2xl p-1 shadow-lg bg-neutral-300/80 shadow-gray-800/5 ring-1 ring-gray-800/[.075] backdrop-blur-xl'>
      <div className="w-96 max-w-96 max-h-96 rounded-2xl overflow-hidden flex flex-col bg-white/90 border border-white/50 shadow-lg shadow-gray-800/5 ring-1 ring-gray-800/[.075] backdrop-blur-xl pointer-events-auto">
        <div className="p-3 flex justify-between items-center bg-black/5">
          <h3 className="text-neutral-700 font-medium">
            Selectors for&nbsp;<em>{window.location.href.replace('https://', '').replace('http://', '').slice(0, 25)}...</em>
          </h3>
          <button
            onClick={onClose}
            className="text-neutral-400 hover:text-neutral-600 p-1 rounded-lg hover:bg-black/5 transition-colors duration-200"
          >
        ...
        <div className="overflow-y-auto flex-1 p-2 space-y-2">
          {items.filter(item => item.isValid).length === 0 ? (
            ...
            items.filter(item => item.isValid).map((item, index) => (
              <div
                key={`${item.timestamp}-${index}`}
                className={`w-full cursor-pointer p-2 rounded-xl flex items-center gap-3 transition-colors group ${activeItem?.timestamp === item.timestamp ? 'bg-black/5' : 'hover:bg-black/5'
                  }`}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-medium text-neutral-900 truncate font-mono">
                    {item.selector}
                  </div>
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg hover:bg-black/5 transition-all duration-200 ${copiedSelectorTimestamp === item.timestamp
                        ? 'text-green-600'
                        : 'text-neutral-500 hover:text-neutral-900'
                      }`}
                    title={copiedSelectorTimestamp === item.timestamp ? "Copied!" : "Copy Selector"}
                  >
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg hover:bg-black/5 transition-all duration-200 ${copiedTimestamp === item.timestamp
                        ? 'text-green-600'
                        : 'text-neutral-500 hover:text-neutral-900'
                      }`}
                    title={copiedTimestamp === item.timestamp ? "Copied!" : "Copy Text"}
                  >
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-black/5 text-neutral-400 hover:text-neutral-900"
                    title="Remove"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
